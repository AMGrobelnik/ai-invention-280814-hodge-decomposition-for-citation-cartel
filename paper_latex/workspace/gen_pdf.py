...
story.append(Paragraph(
    ...
    "1-chain f : E → ℝ. A subgroup S ⊆ V induces a subflow f|_S of all edges within S. For each "
...
story.append(Paragraph(
    ...
    "components. Using the incidence matrix B₁ (dimensions V × E), where B₁(v,e) = +1 if v is the "
    "head of edge e, −1 if v is the tail, and 0 otherwise:",
    body_style
))
story.append(Paragraph(
    "The " + b("gradient component") + " satisfies f_grad = B₁ᵀφ for some potential φ : V → ℝ, "
...
story.append(Paragraph("φ* = argmin_φ ||f − B₁ᵀφ||²", eq_style))
story.append(Paragraph(
    "This is equivalent to solving the normal equation (B₁B₁ᵀ)φ = B₁f. Using "
    "scipy.sparse.linalg.lsqr [13], we efficiently compute φ* in O(V·E) time. The gradient component "
    "is then f_grad = B₁ᵀφ*, and the residual f_residual = f − f_grad.",
    body_style
))
...
story.append(Paragraph(
    ...
    "boundary operator B₂. Recent work (HLSAD, 2025) validates this approach on simplicial "
