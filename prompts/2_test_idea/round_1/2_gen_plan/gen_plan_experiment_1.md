...
approach: >-
  Load the weighted citation graph from the DATASET. For Hodge decomposition: construct the incidence matrix B1 (nodes × edges)
  and boundary matrix B2 (edges × triangles); solve the least-squares system min||B1 f_grad - f|| using scipy.sparse.linalg.lsqr
  ...
  = excess citation fraction relative to uniform null model (without full dcSBM fitting). Rank all subgroups descending by
  each scoring method. Obtain Clarivate suppressed journals list (ground truth). For each method, compute precision@5, precision@10,
  precision@20, and AUC-ROC by treating the subgroup rank as a continuous score. Report tabular comparison. Diagnose the result:
  if CVS > baselines, confirm the hypothesis; if CVS ≈ reciprocity ratio, conclude Hodge decomposition adds no information;
  if CVS < baselines, the curl model may not describe cartel structure.
depends_on: []
</artifact_direction>
...
### [3] SKILL-INPUT — aii-web-tools · 2026-07-09 00:27:00 UTC
